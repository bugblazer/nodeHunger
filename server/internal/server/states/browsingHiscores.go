...
func (b *BrowsingHiscores) OnEnter() {
	...
	if err != nil {
		b.logger.Printf("Error getting top %d scores from the rank %d: %v", limit, offset, err)
		b.client.SocketSend(packets.NewDenyResponse("Failed to get the top scores - please try again later."))
		return
	}
